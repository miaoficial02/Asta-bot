...
// Mensaje QR y Code: compacto, llamativo y con emojis
let rtx = `*🔗 SubBot QR*\n\nEscanea este QR con otro celular para conectar tu SubBot.\n\n1️⃣ WhatsApp > Dispositivos vinculados\n2️⃣ Vincular un dispositivo\n3️⃣ Escanea este código aquí\n\n⏳ Expira en 45s\n🚫 No uses tu cuenta principal\n\n✨ Conéctate gratis con nbot ✨`

let rtx2 = `*🔑 SubBot Código*\n\nConecta tu SubBot con este código:\n\n1️⃣ WhatsApp > Dispositivos vinculados\n2️⃣ Vincular con número de teléfono\n3️⃣ Ingresa el código de abajo\n\n🚫 No uses tu cuenta principal\n\n✨ Conéctate gratis con nbot ✨`
...
export async function yukiJadiBot(options) {
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    const connectionOptions = {
      ...
      msgRetry,
      msgRetryCache,
      browser: mcode ? ['Ubuntu', 'Chrome', '110.0.5585.95'] : ['ASTA', 'Chrome', '2.0.0'],
      version: version,
    ...
    async function connectionUpdate(update) {
      ...
      if (connection === 'close') {
        if (reason === 428) {
          console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La conexión (+${path.basename(pathYukiJadiBot)}) falló por error 428\n╰───────────────────────────────────────────────────⟡`))
          await creloadHandler(true).catch(console.error)
        }
        if (reason === 408) {
          console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La conexión (+${path.basename(pathYukiJadiBot)}) se cerró por error 408\n╰───────────────────────────────────────────────────⟡`))
          await creloadHandler(true).catch(console.error)
        }
        if (reason === 440) {
          console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La conexión (+${path.basename(pathYukiJadiBot)}) falló por error 440\n╰───────────────────────────────────────────────────⟡`))
          try {
            if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(pathYukiJadiBot)}@s.whatsapp.net`, { text: '*HEMOS DETECTADO UNA NUEVA SESIÓN, BORRE LA NUEVA SESIÓN PARA CONTINUAR*\n\n> *Si no reconoce la sesión, ignore este mensaje.*' }, { quoted: m }) : null
          } catch (error) {
            console.error(chalk.bold.yellow(`Error 440 no se pudo enviar mensaje a: +${path.basename(pathYukiJadiBot)}`))
          }
        }
        if (reason == 405 || reason == 401) {
          console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ La sesión (+${path.basename(pathYukiJadiBot)}) fue cerrada por error 405/401\n╰───────────────────────────────────────────────────⟡`))
          try {
        ...
        if (reason === 500) {
          console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡\n┆ Conexión perdida en la sesión (+${path.basename(pathYukiJadiBot)})\n╰───────────────────────────────────────────────────⟡`))
      ...
      if (connection == `open`) {
        ...
        m?.chat ? await conn.sendMessage(m.chat, {
          text: args[0]
            ...
            : `@${m.sender.split('@')[0]}, genial ya eres parte de nuestra red Sub-Bot.\n\n✨ Conéctate gratis con nbot ✨`,
          mentions: [m.sender]
        }, { quoted: m }) : null;
      }
    }
